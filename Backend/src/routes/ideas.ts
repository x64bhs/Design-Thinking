...
import {
  ...
  expandIdeaRequestSchema,
  generateIdeasRequestSchema,
  generateOneIdeaRequestSchema,
  ideaSchema,
...
async function withStubFallback<T>(run: () => Promise<T>, fallback: () => T | Promise<T>) {
  try {
    return await run()
  } catch (err) {
...
function parseModelJson<T>(raw: string, schema: z.ZodType<T>): T {
  ...
  if (!out.success) throw new ApiError(502, 'Model JSON did not match schema')
  return out.data
}
...
async function generateIdeasFromModel(input: z.infer<typeof generateIdeasRequestSchema>) {
  ...
  const prompt = [
    ...
    'Problem statements must be descriptive: write 2 full sentences (around 25-45 words total).',
    'Sentence 1 should explain the concrete pain point. Sentence 2 should explain context/impact.',
    'Do NOT include implementation details, feature lists, or tech stack in the problem statement.',
  ...
  try {
    ...
    const parsed = parseModelJson(
      text,
      z.object({ ideas: z.array(ideaSchema.omit({ id: true })).min(1) }),
    )
...
async function generateOneIdeaFromModel(input: z.infer<typeof generateOneIdeaRequestSchema>) {
  const exclude = (input.excludeTitles ?? []).slice(0, 50)
  const prompt = [
    'You are the core intelligence behind a project idea generation system.',
    ...
    'Problem statement must be descriptive: write 2 full sentences (around 25-45 words total).',
    'Sentence 1 should explain the concrete pain point. Sentence 2 should explain context/impact.',
  ...
  try {
    ...
    const parsed = parseModelJson(
      text,
      z.object({ idea: ideaSchema.omit({ id: true }) }),
    )
...
async function expandIdeaFromModel(input: z.infer<typeof expandIdeaRequestSchema>) {
  const prompt = [
    'You are the core intelligence behind a project idea generation system.',
    '',
    ...
    '- Implementation steps: 6-10 concrete build actions in chronological order.',
    '- Implementation steps MUST be development tasks (e.g., schema design, API endpoints, UI screens, integration, deployment).',
    '- Do NOT repeat core feature names verbatim in implementation steps.',
  ].filter(Boolean).join('\n')
  ...
  try {
    ...
    const parsed = parseModelJson(
      text,
      z.object({ blueprint: blueprintSchema }),
    )
...
async function refineIdeaFromModel(input: z.infer<typeof refineIdeaRequestSchema>) {
  const prompt = [
    'You are the core intelligence behind a project idea generation system.',
...
ideasRouter.post('/generate', asyncRoute(async (req, res) => {
  ...
  const ideas = await withStubFallback(
    () => generateIdeasFromModel(parsed.data),
    () => stubGenerateIdeas(parsed.data, parsed.data.count),
  )
...
ideasRouter.post('/generate-one', asyncRoute(async (req, res) => {
  ...
  if (!parsed.success) throw new ApiError(400, 'Invalid input')
  const idea = await withStubFallback(
    () => generateOneIdeaFromModel(parsed.data),
    () => stubGenerateOneIdea(parsed.data),
  )
...
ideasRouter.post('/expand', asyncRoute(async (req, res) => {
  ...
  if (!parsed.success) throw new ApiError(400, 'Invalid input')
  const blueprint = await withStubFallback(
    () => expandIdeaFromModel(parsed.data),
    () => stubExpandIdea(parsed.data),
  )
...
ideasRouter.post('/refine', asyncRoute(async (req, res) => {
  ...
  if (!parsed.success) throw new ApiError(400, 'Invalid input')
  const blueprint = await withStubFallback(
    () => refineIdeaFromModel(parsed.data),
    () => stubRefineBlueprint(parsed.data),
  )
  return res.json({ blueprint })
}))
...
ideasRouter.post('/sessions', asyncRoute(async (req, res) => {
  ...
  if (!payload.success) throw new ApiError(400, 'Invalid input')

  const now = new Date().toISOString()
  const sessionId = id('ses')
  const db = await getDb()
  await db.run(
    `INSERT INTO idea_sessions (id, user_id, input_json, ideas_json, selected_idea_json, blueprint_json, created_at, updated_at)
     VALUES (?,?,?,?,?,?,?,?)`,
    sessionId,
    userId,
    JSON.stringify(payload.data.input),
    JSON.stringify(payload.data.ideas),
    payload.data.selectedIdea ? JSON.stringify(payload.data.selectedIdea) : null,
    payload.data.blueprint ? JSON.stringify(payload.data.blueprint) : null,
    now,
    now,
  )
...
ideasRouter.get('/sessions', asyncRoute(async (req, res) => {
  ...
  const rows = await db.all<{
    id: string
    input_json: string
    selected_idea_json: string | null
    created_at: string
    updated_at: string
  }[]>(
    'SELECT id, input_json, selected_idea_json, created_at, updated_at FROM idea_sessions WHERE user_id = ? ORDER BY updated_at DESC LIMIT 50',
    userId,
  )
  return res.json({
    sessions: rows.map((row) => ({
      id: row.id,
      title: row.selected_idea_json
        ? ((JSON.parse(row.selected_idea_json) as { title?: unknown }).title as string | undefined) ?? null
        : null,
...
ideasRouter.get('/sessions/:id', asyncRoute(async (req, res) => {
  ...
  const row = await db.get<{
    ...
    `SELECT input_json, ideas_json, selected_idea_json, blueprint_json, created_at, updated_at
     FROM idea_sessions
...
ideasRouter.post('/share', asyncRoute(async (req, res) => {
  ...
  const db = await getDb()
  await db.run(
    'INSERT INTO share_links (id, user_id, blueprint_json, created_at) VALUES (?,?,?,?)',
    shareId,
    userId,
    JSON.stringify(parsed.data.blueprint),
...
ideasRouter.get('/share/:shareId', asyncRoute(async (req, res) => {
  ...
  if (!parsed.success) throw new ApiError(400, 'Invalid input')
  const db = await getDb()
  const row = await db.get<{ blueprint_json: string }>(
    'SELECT blueprint_json FROM share_links WHERE id = ?',
    parsed.data.shareId,
  )
  if (!row) throw new ApiError(404, 'Not found')
