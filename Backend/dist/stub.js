...
function makeIdea(i, input) {
    ...
    const problemStatement = angle === 'accessibility'
...
function avoidTitles(title, exclude) {
    if (!exclude?.length)
        return title;
    const taken = new Set(exclude.map((t) => t.trim().toLowerCase()).filter(Boolean));
    if (!taken.has(title.trim().toLowerCase()))
        return title;
    let k = 2;
    while (taken.has(`${title} ${k}`.toLowerCase()) && k < 99)
        k++;
    return `${title} ${k}`;
}
...
export function stubGenerateIdeas(input, count) {
    const ideas = Array.from({ length: Math.max(1, count) }, (_, i) => makeIdea(i, input));
    return ideas;
}
export function stubGenerateOneIdea(input) {
