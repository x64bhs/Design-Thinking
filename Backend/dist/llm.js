...
export async function generateJsonTextOrThrow(prompt) {
...
async function groqGenerateTextOrThrow(prompt) {
    const apiKey = process.env.GROQ_API_KEY;
    if (!apiKey)
        throw new ApiError(502, 'GROQ_API_KEY is not configured');
    ...
    const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
        ...
        body: JSON.stringify({
            ...
            messages: [
                {
                    role: 'system',
                    content: 'You must output ONLY valid JSON. No markdown, no code fences, no extra text.',
                },
    ...
    if (!text || !text.trim())
        throw new ApiError(502, 'Groq returned empty output');
    return text;
}
async function geminiGenerateTextOrThrow(prompt) {
